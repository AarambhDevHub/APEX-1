"""
APEX-1 LoRA / PEFT fine-tuning CLI.
...
from apex.model.lora import apply_lora_adapters, load_lora_adapters
...
def build_model_with_optional_base_checkpoint(
    ...
    """Load a base checkpoint safely, then inject LoRA.

    A normal base checkpoint has keys like ``blocks.0.attn.W_Q.weight``.
    A LoRA-wrapped model has keys like ``blocks.0.attn.W_Q.base_layer.weight``.
    To avoid key mismatch, we create the base model first with PEFT disabled,
    load the checkpoint, then inject adapters.
    """
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Fine-tune APEX-1 with LoRA/PEFT")
    ...
    parser.add_argument("--output-dir", type=str, default="checkpoints/lora", help="Adapter save dir")
    ...
    logger.info("Done. Adapter saved to %s", result["adapter_path"])
