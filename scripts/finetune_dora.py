"""
APEX-1 DoRA PEFT fine-tuning CLI.

v2.8.0 adds Weight-Decomposed LoRA:

    frozen base model + trainable LoRA direction update + trainable magnitude
...
def build_model_with_optional_base_checkpoint(
    ...
    """Load a base checkpoint safely, then inject DoRA/QDoRA adapters."""
    ...
    config.peft.method = method
    apply_lora_adapters(model, config.peft)
    return model


def main() -> None:
    parser = argparse.ArgumentParser(description="Fine-tune APEX-1 with DoRA PEFT")
    parser.add_argument("--config", type=str, default="configs/apex1_tiny_dora.yaml")
    ...
    parser.add_argument("--output-dir", type=str, default="outputs/dora-test", help="Adapter save dir")
    parser.add_argument("--max-steps", type=int, default=None, help="Override max train steps")
    parser.add_argument("--method", choices=["dora", "qdora"], default=None, help="Override PEFT method")
    parser.add_argument("--dry-run", action="store_true", help="Use synthetic data")
    args = parser.parse_args()

    config = APEXConfig.from_yaml(args.config)
    method = args.method or config.peft.method or "dora"
    if method not in {"dora", "qdora"}:
        raise ValueError("finetune_dora.py expects method dora or qdora")

    config.peft.enabled = True
    config.peft.method = method
    config.validate()

    model = build_model_with_optional_base_checkpoint(config, args.checkpoint, method)

    if args.adapter:
        load_lora_adapters(model, args.adapter, strict=False)

    logger.info("DoRA modules inserted: %d", count_dora_modules(model))
    logger.info("PEFT summary: %s", peft_parameter_summary(model))
    ...
    logger.info("Done. DoRA adapter saved to %s", result["adapter_path"])
