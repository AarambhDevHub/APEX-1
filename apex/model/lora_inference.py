"""
APEX-1 LoRA inference loading helpers.
...
merge/export flows:
    ...
    base checkpoint + LoRA adapter -> generation model
    base checkpoint + LoRA adapter -> merged plain APEX checkpoint
...
4. re-enable PEFT and inject LoRA wrappers
...
def build_base_model_then_apply_lora(
    ...
    Returns:
        A model with LoRA wrappers attached when ``config.peft.enabled`` is true.
    """
...
def load_lora_model_for_inference(
    ...
    """Build an inference-ready model and load a LoRA adapter.
